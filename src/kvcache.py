...
def find_candidate_pred_tokens(input_ids, max_ngram_size=3, num_pred_tokens=10, 
    ...
    增强版：支持检索库
    搜索优先级：
    1. 检索库（如果提供）- 包含持久化前缀库、动态历史库、被拒绝tokens库
    2. 回退到原始输入序列搜索
    ...
    if retrieval_cache is not None:
        candidate = retrieval_cache.find_candidate_tokens(input_ids)
        if len(candidate) > 0:
            return candidate
...
class KVCacheModel():
    ...
    def _generate_with_pld(self, prefix: torch.Tensor, gamma: int, max_ngram_size: int = 3,
        ...
        for step in range(gamma):
            ...
            # Step 1: Find PLD candidate tokens (使用检索库)
            ...
            if self._retrieval_cache is not None and accept_length > 0:
                self._retrieval_cache.add_to_dynamic_cache(valid_tokens[0])
